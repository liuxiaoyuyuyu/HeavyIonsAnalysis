...
#ifdef MyClass2_cxx
MyClass2::MyClass2(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("output_working.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("output_working.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("output_working.root:/analyzer");
...
Int_t MyClass2::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t MyClass2::LoadTree(Long64_t entry)
{
...
void MyClass2::Show(Long64_t entry)
{
...
Int_t MyClass2::Cut(Long64_t entry)
{
